...
try {
    ...
    $check = $pdo->prepare("SELECT COUNT(*) FROM registrations WHERE email = ?");
    $check->execute([$email]);
    ...
    $stmt = $pdo->prepare("INSERT INTO registrations (first_name, last_name, email, registration_type, agreed_terms) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$firstName, $lastName, $email, $type, $agreed]);

    // Admin notification
    $adminEmail = "dis@skyalphahd.com";
